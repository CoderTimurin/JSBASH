...
import getFileTypeIcon from '@JSBASH/dashboard/lib/utils/getFileTypeIcon.js'
...
function JSBASHDashboardFileIcon ({ type }) {
...
export default function FileList ({ JSBASH }) {
  const JSBASHFiles = JSBASH.store.state.files
  const JSBASHFilesArray = Object.keys(JSBASHFiles).map((id) => JSBASHFiles[id])

  return (
    <View style={styles.container}>
      <FlatList
        data={JSBASHFilesArray}
          ...
          return (
            <View style={styles.item}>
              {item.type === 'image' ? (
                ...
                <JSBASHDashboardFileIcon type={item.type} />
              )}
