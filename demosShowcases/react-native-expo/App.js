...
import FilePicker  from '@JSBASH/react-native'
import  useJSBASH  from '@JSBASH/react/lib/useJSBASH'
...
export default function App () {
  ...
  const JSBASH = useJSBASH(() => {
    return new JSBASH({ autoProceed: true, debug: true })
      .use(Tus, {
  ...
  useEffect(() => {
    JSBASH.on('upload-progress', (file, progress) => {
      setState({
        progress: progress.bytesUploaded,
        total: progress.bytesTotal,
        totalProgress: JSBASH.state.totalProgress,
        uploadStarted: true,
      })
    })
    JSBASH.on('upload-success', () => {
      // console.log(file.name, response)
    })
    JSBASH.on('complete', (result) => {
      setState({
    ...
    JSBASH.on('info-visible', () => {
      const { info } = JSBASH.getState()
      setState({
        info,
      })
      console.log('JSBASH-info:', info)
    })
    JSBASH.on('info-hidden', () => {
      setState({
        info: null,
      })
    })
  ...
  return (
    ...
    >
      <Text
        style={styles.title}
      >
        JSBASH in React Native
      </Text>
      <View style={{ alignItems: 'center' }}>
        <Image
          style={styles.logo}
          // eslint-disable-next-line global-require
          source={require('./assets/JSBASH-logo.png')}
        />
      </View>
