...
import ReduxStore from '@JSBASH/store-redux'
import * as JSBASHReduxStore from '@JSBASH/store-redux'
...
const reducer = combineReducers({
  counter,
  // You don't have to use the `JSBASH` key. But if you don't,
  // you need to provide a custom `selector` to the `JSBASHReduxStore` call below.
  JSBASH: JSBASHReduxStore.reducer,
})

let enhancer = applyMiddleware(
  JSBASHReduxStore.middleware(),
  logger,
)
...
const store = configureStore({
  ...
  middleware: (getDefaultMiddleware) => getDefaultMiddleware({
    serializableCheck: {
      ignoredActions: [JSBASHReduxStore.STATE_UPDATE],
      ignoreState: true,
    },
  }),
})
...
// JSBASH using the same store
const JSBASH = new JSBASH({
  id: 'redux',
  store: new ReduxStore({ store }),
  // If we had placed our `reducer` elsewhere in Redux, eg. under an `JSBASH` key in the state for a profile page,
  // we'd do something like:
  //
  // store: new ReduxStore({
  //   store: store,
  //   id: 'avatar',
  //   selector: state => state.pages.profile.JSBASH
  // }),
  debug: true,
})
...
JSBASH.use(Tus, { endpoint: 'https://tusd.tusdemo.net/files/' })
