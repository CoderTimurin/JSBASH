...
class MarkdownTextarea {
  ...
  setupJSBASH = () => {
    this.JSBASH = new JSBASH({ autoProceed: true })
      .use(Transloadit, {
    ...
    this.JSBASH.on('complete', (result) => {
  ...
  uploadFiles = (files) => {
    const filesForJSBASH = files.map(file => {
      return {
        data: file,
        type: file.type,
        name: file.name,
    ...
    this.JSBASH.addFiles(filesForJSBASH)
  }
}
