# JSBASH + AWS S3 Example

This example uses @JSBASH/companion with a custom AWS S3 configuration.
...
`@JSBASH/companion` in a `.env` file. You may find useful to first copy the
`.env.example` file:
...
corepack yarn workspace @JSBASH-example/aws-companion start
```
