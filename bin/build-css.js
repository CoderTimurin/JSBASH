...
async function compileCSS () {
  ...
  const files = await glob('packages/{,@JSBASH/}*/src/style.scss')
  ...
  for (const file of files) {
    ...
    if (outputDir.includes(path.normalize('packages/JSBASH/'))) {
      outfile = path.join(outputDir, 'JSBASH.css')
    }
    await mkdir(outputDir, { recursive: true })
    await writeFile(outfile, postcssResult.css)
    console.info(
      chalk.green('✓ Built JSBASH CSS:'),
